const { web3 } = window
const selectedAddress = web3.eth.defaultAccount // metamask account

$(document).ready(function() {
    const productRegistryContractAddress = '0xE81C44C7480445e74F2705c97D8704Da5b0E5979';
	...
	async function showTable() {
		...
		if (window.ethereum) // matamask 설치되어 있으면
			try {
				await window.ethereum.enable();
			} catch (err) {
                return showError("Access to your Ethereum account rejected.");
			}
		if (typeof web3 === 'undefined') // 설치되어 있지 않으면
		...
		contract.getNumOfProducts(function(err, result) { //smart contract function => getNumOfProducts
			if (err)
